...
# ---------------------------------------------------------------------------
# P38: Fisher-Rao Metric for Constellation Distance
# ---------------------------------------------------------------------------

@dataclass(frozen=True)
class ConstellationDistance:
    """Information-geometric distance between two constellation configurations.

    Uses the Fisher-Rao metric on the space of coverage distributions:
    each constellation induces a discrete probability distribution p_C(k)
    over coverage counts (fraction of grid points with exactly k visible
    satellites).

    The Fisher-Rao distance is the geodesic distance on the statistical
    manifold of discrete distributions. For discrete distributions, it
    equals 2 * arccos(Bhattacharyya coefficient).

    Attributes:
        fisher_rao_distance: Geodesic distance on the statistical manifold.
        hellinger_distance: Hellinger distance H = sqrt(1 - BC).
        coverage_correlation: Pearson correlation of coverage count vectors.
        kl_divergence: KL divergence D_KL(p || q) (asymmetric, p -> q).
        is_significantly_different: True if Fisher-Rao distance > threshold.
    """
    fisher_rao_distance: float
    hellinger_distance: float
    coverage_correlation: float
    kl_divergence: float
    is_significantly_different: bool


def compute_constellation_distance(
    coverage_counts_a: list,
    coverage_counts_b: list,
    significance_threshold: float = 0.5,
) -> ConstellationDistance:
    """Compute Fisher-Rao distance between two constellation coverage distributions.

    Each input is a list of per-grid-point satellite visibility counts.
    These are converted to discrete probability distributions over coverage
    levels, then compared using information-geometric metrics.

    Args:
        coverage_counts_a: Per-grid-point visibility counts for constellation A.
            E.g. [0, 1, 3, 2, 0, 1, ...] where each entry is the number of
            satellites visible from that grid point.
        coverage_counts_b: Per-grid-point visibility counts for constellation B.
            Must have the same length as coverage_counts_a.
        significance_threshold: Fisher-Rao distance threshold for significance.

    Returns:
        ConstellationDistance with Fisher-Rao, Hellinger, correlation, KL metrics.
    """
    counts_a = np.array(coverage_counts_a, dtype=np.int64)
    counts_b = np.array(coverage_counts_b, dtype=np.int64)

    if len(counts_a) == 0 or len(counts_b) == 0:
        return ConstellationDistance(
    ...
    # Build coverage distributions p(k) and q(k)
    max_k = max(int(np.max(counts_a)) if len(counts_a) > 0 else 0,
                int(np.max(counts_b)) if len(counts_b) > 0 else 0)

    n_bins = max_k + 1
    p = np.zeros(n_bins, dtype=np.float64)
    q = np.zeros(n_bins, dtype=np.float64)

    for k in range(n_bins):
        p[k] = float(np.sum(counts_a == k)) / len(counts_a)
        q[k] = float(np.sum(counts_b == k)) / len(counts_b)
    ...
    fisher_rao = 2.0 * math.acos(bc)

    # Hellinger distance: H = sqrt(1 - BC)
    hellinger = math.sqrt(max(0.0, 1.0 - bc))

    # Pearson correlation of raw coverage count vectors
    if len(counts_a) == len(counts_b) and len(counts_a) > 1:
        mean_a = float(np.mean(counts_a))
        mean_b = float(np.mean(counts_b))
        std_a = float(np.std(counts_a))
        std_b = float(np.std(counts_b))
        if std_a > 1e-15 and std_b > 1e-15:
            cov_ab = float(np.mean((counts_a - mean_a) * (counts_b - mean_b)))
            correlation = cov_ab / (std_a * std_b)
        else:
            correlation = 1.0 if std_a < 1e-15 and std_b < 1e-15 else 0.0
    else:
        correlation = 0.0

    # KL divergence: D_KL(p || q) = sum p_k * log(p_k / q_k)
    # Add small epsilon to avoid log(0)
    eps = 1e-300
    kl_div = 0.0
    for k in range(n_bins):
        if p[k] > eps:
            kl_div += p[k] * math.log(p[k] / max(q[k], eps))

    is_significant = fisher_rao > significance_threshold

    return ConstellationDistance(
        fisher_rao_distance=fisher_rao,
        hellinger_distance=hellinger,
        coverage_correlation=correlation,
