...
def compute_dop(
    ...
    q = np.array(q_list)
    # Guard against slightly negative diagonal elements from numerical errors
    gdop = float(np.sqrt(max(0.0, q[0, 0] + q[1, 1] + q[2, 2] + q[3, 3])))
    pdop = float(np.sqrt(max(0.0, q[0, 0] + q[1, 1] + q[2, 2])))
    hdop = float(np.sqrt(max(0.0, q[0, 0] + q[1, 1])))
    vdop = float(np.sqrt(max(0.0, q[2, 2])))
    tdop = float(np.sqrt(max(0.0, q[3, 3])))
