...
**Version**: 2.0 — Effective 2026-02-13

## 1. Definitions

- **"Licensor"**: Jeroen Visser, sole copyright holder.
- **"Licensee"**: The legal entity purchasing a commercial license.
- **"Software"**: The commercially licensed files listed in Section 2.
- **"Open Core"**: The MIT-licensed files listed in `LICENSE`. These are not
  subject to this agreement.

## 2. Covered files

The following files are covered by this commercial license. They are **not**
covered by the MIT License in `LICENSE`. The file header is the authoritative
source of which license applies to each file.

**Domain modules** (`packages/pro/src/humeris/domain/`):
...
- `cascade_analysis.py` — Cascade/fragmentation indicators, SIR model
- `communication_analysis.py` — Network capacity analysis
- `competing_risks.py` — Competing-risks population dynamics
...
- `functorial_composition.py` — Functorial force composition
- `gramian_reconfiguration.py` — Gramian-guided reconfiguration
- `graph_analysis.py` — Graph-theoretic ISL (Fiedler, fragmentation, Hodge)
- `gravity_field.py` — Cunningham V/W spherical harmonic gravity (EGM96 70x70)
- `hazard_reporting.py` — NASA-STD-8719.14 hazard classification, breakup potential
- `hodge_cusum.py` — Hodge-CUSUM topology detector
...
- `koopman_conjunction.py` — Koopman-spectral conjunction screening
- `koopman_propagation.py` — Koopman operator propagation via Dynamic Mode Decomposition
...
- `maneuver_detection.py` — Innovation-based maneuver detection (CUSUM, EWMA, chi-squared)
...
**Adapters** (`packages/pro/src/humeris/adapters/`):
...
- `viewer_server.py` — Interactive viewer server with analysis dispatch

**Tests** (`tests/`):

All test files whose header references `COMMERCIAL-LICENSE.md`.

**Data** (`packages/pro/src/humeris/data/`):

All data files in this directory.

Copyright (c) 2026 Jeroen Visser. All rights reserved.

## 3. Free use (non-commercial)

You may use, copy, and modify the covered files **at no cost** for:

- Personal projects
- Academic research and education
- Evaluation, testing, and prototyping
- Open-source projects that do not generate revenue
- Any other non-commercial purpose

**Clarification — academic use**: Use at a publicly funded academic institution
for research purposes is non-commercial, regardless of funding source (grants,
government funding, industry-sponsored research collaborations).

**Clarification — evaluation**: You may evaluate the Software for up to 90 days
at a for-profit entity to determine whether to purchase a commercial license.
During evaluation, the Software may not be used in production, for revenue-
generating activities, or to provide services to third parties.

## 4. Commercial use

Commercial use means using the Software in connection with any activity that
generates or is intended to generate revenue, directly or indirectly. This
includes, but is not limited to:

- Incorporating the Software into a product or service offered for a fee
- Using the Software to provide paid consulting, analysis, or engineering
  services to third parties
- Using the Software as an internal tool at a for-profit entity (beyond the
  90-day evaluation period in Section 3)
- Offering the Software (or a derivative) as a hosted service (SaaS)

**Consumer exception**: If you are a natural person acting outside your trade,
business, or profession, you may use the Software free of charge under Section 3.
...
non-transferable** (except as provided in Section 12) license to:

- **Use** the Software for the Licensee's own commercial purposes
- **Copy** the Software for internal use, backup, and disaster recovery
- **Modify** the Software and create derivative works for the Licensee's
  own commercial purposes
- **Deploy** the Software as part of the Licensee's products, services, or
  internal infrastructure, including SaaS offerings

The license does **not** include the right to:

- **Redistribute** the Software (in source or binary form) as a standalone
  product or library
- **Sublicense** the Software to third parties
- **Offer** the Software itself (as opposed to a product built with it) as a
  service

Contractors, consultants, and agents of the Licensee may use the Software on
behalf of the Licensee under this license, provided such use is exclusively for
the Licensee's benefit and the Licensee remains responsible for compliance.

## 6. Pricing

| Tier | Eligibility | One-time license fee |
|------|-------------|---------------------|
| **Startup** | < 50 employees AND ≤ EUR 10M annual revenue | **EUR 2,000** |
| **Enterprise** | ≥ 50 employees OR > EUR 10M annual revenue | **EUR 7,500** |
| **Government & Defense** | Government agencies, military, or entities where > 50% of annual revenue derives from government or defense contracts | **EUR 15,000** |

Employee count and revenue are measured at the purchasing legal entity and its
majority-owned affiliates (> 50% ownership), based on the most recent audited
financial statements or annual accounts at the time of purchase.

The applicable tier is **locked at the date of purchase**. If the Licensee later
grows beyond the original tier's eligibility criteria, the existing license
remains valid and the Licensee is not required to pay the difference. Future
purchases of additional modules are priced at the then-applicable tier.

## 7. Optional: Support & Updates subscription

**EUR 1,500/year** (all tiers). Includes:

- Priority bug fixes
- Access to new commercial modules released during the subscription period
- Direct communication channel for bug reports and technical questions
...
files and destroy all copies. Sections 8 (Warranty disclaimer), 9 (Limitation
of liability), 11 (Governing law), and 14 (Export control) survive termination.
...
30 days. Failure to respond constitutes a material breach under Section 10.
...
Contact via the project repository at
**https://github.com/pljeroen/humeris** or via
**https://www.linkedin.com/in/jeroen-visser-253154a9/** with:

- Your company name, registered address, and applicable tier
- Intended use (brief description)
- For Government & Defense tier: nature of the contracting entity

You will receive an invoice and a signed license agreement. Payment via bank
transfer. License is effective upon payment.

## 16. Relationship to this document

This document describes the commercial licensing terms for the covered files.
It is **not** itself a binding agreement. A binding license agreement is
executed separately upon purchase (signed by both parties). Until such agreement
is executed, no commercial use rights are granted beyond the evaluation period
in Section 3.

## 17. Open core files

All files listed in the "MIT-Licensed Files" section of `LICENSE` are MIT-
licensed. They are not subject to this agreement and may be used freely under
